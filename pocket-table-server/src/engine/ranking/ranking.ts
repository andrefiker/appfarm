...
export type HandCategory =
  ...
  | 'FOUR_OF_A_KIND'
  | 'STRAIGHT_FLUSH';
export type RankedHand = Readonly<{
  category: HandCategory;
  categoryValue: number;
  rankVector: readonly number[];
  winningFive: readonly Card[];
}>;
...
function sorted(cards: readonly Card[]): Card[] {
  return [...cards].sort(
    (a, b) => value[b.rank] - value[a.rank] || a.id.localeCompare(b.id)
  );
}
function straightHigh(cards: readonly Card[]): number | undefined {
  const unique = [...new Set(cards.map(c => value[c.rank]))].sort(
    (a, b) => b - a
  );
...
export function rankFive(cards: readonly Card[]): RankedHand {
  if (cards.length !== 5 || new Set(cards.map(c => c.id)).size !== 5)
    throw new Error('rankFive requires five unique cards');
  const groups = [...new Map<number, Card[]>().entries()];
  const byRank = new Map<number, Card[]>();
  for (const c of cards)
    byRank.set(value[c.rank], [...(byRank.get(value[c.rank]) ?? []), c]);
  const grouped = [...byRank.entries()].sort(
  ...
  const make = (
    ...
    winningFive: readonly Card[] = sorted(cards)
  ): RankedHand =>
    Object.freeze({
      category,
      categoryValue: categoryValue[category],
      rankVector: Object.freeze([...vector]),
      winningFive: Object.freeze([...winningFive]),
    });
  if (flush && sh !== undefined)
    return make('STRAIGHT_FLUSH', [sh], orderStraight(cards, sh));
  if (grouped[0]![1].length === 4)
    return make('FOUR_OF_A_KIND', [grouped[0]![0], grouped[1]![0]]);
  if (grouped[0]![1].length === 3 && grouped[1]![1].length === 2)
    return make('FULL_HOUSE', [grouped[0]![0], grouped[1]![0]]);
  if (flush)
    return make(
      'FLUSH',
      sorted(cards).map(c => value[c.rank])
    );
  if (sh !== undefined) return make('STRAIGHT', [sh], orderStraight(cards, sh));
  if (grouped[0]![1].length === 3)
    return make('THREE_OF_A_KIND', [
      grouped[0]![0],
      ...grouped.slice(1).map(g => g[0]),
    ]);
  if (grouped[0]![1].length === 2 && grouped[1]![1].length === 2)
    return make('TWO_PAIR', [grouped[0]![0], grouped[1]![0], grouped[2]![0]]);
  if (grouped[0]![1].length === 2)
    return make('PAIR', [grouped[0]![0], ...grouped.slice(1).map(g => g[0])]);
  return make(
    'HIGH_CARD',
    sorted(cards).map(c => value[c.rank])
  );
...
export function compareHands(a: RankedHand, b: RankedHand): number {
  if (a.categoryValue !== b.categoryValue)
    return a.categoryValue - b.categoryValue;
  for (
    let i = 0;
    i < Math.max(a.rankVector.length, b.rankVector.length);
    i += 1
  ) {
...
export function rankBest(cards: readonly Card[]): RankedHand {
  if (
    ...
    throw new Error('rankBest requires 5–7 unique cards');
