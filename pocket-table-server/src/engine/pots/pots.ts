...
export function buildPots(
  ...
  if (contributions.some(c => !Number.isSafeInteger(c.amount) || c.amount < 0))
    throw new Error('Invalid contribution');
  const levels = [
    ...new Set(contributions.map(c => c.amount).filter(n => n > 0)),
  ].sort((a, b) => a - b);
  const pots: Pot[] = [];
  ...
  for (const level of levels) {
    ...
    if (involved.length < 2) {
      ...
      if (only)
        returned[only.playerId] = (returned[only.playerId] ?? 0) + slice;
      continue;
    }
    pots.push(
      Object.freeze({
        ...
        eligibleIds: Object.freeze(
          involved.filter(c => !c.folded).map(c => c.playerId)
        ),
        contributorIds: Object.freeze(involved.map(c => c.playerId)),
      })
    );
  }
...
export function splitPot(
  ...
  if (!winnerIds.length || amount < 0 || !Number.isInteger(amount))
    throw new Error('Invalid split');
  const out: Record<string, number> = Object.fromEntries(
    winnerIds.map(id => [id, Math.floor(amount / winnerIds.length)])
  );
  const ordered = clockwiseOrder.filter(id => winnerIds.includes(id));
  for (let i = 0; i < amount % winnerIds.length; i += 1) {
    const id = ordered[i]!;
    out[id] = (out[id] ?? 0) + 1;
  }
