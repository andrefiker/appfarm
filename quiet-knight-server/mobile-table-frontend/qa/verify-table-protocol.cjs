...
let harness=fs.readFileSync('qa/fixtures/reconnect.cjs','utf8').split("const before=harness")[0];
...
const h=harness('src/use-room-realtime.ts'),s=h.instances[0];s.open();assert.ok(!s.url.includes('private-fixture'));assert.equal(s.sent[0].type,'presence.hello');assert.equal(s.sent[0].seat_token,'private-fixture');
