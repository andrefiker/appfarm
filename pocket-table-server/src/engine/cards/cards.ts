...
export function card(rank: Rank, suit: Suit): Card {
  return Object.freeze({ id: `${rank}${suit}`, rank, suit });
}
...
export function shuffle(
  deck: readonly Card[],
  random: RandomSource = mulberry32(0xc0ffee)
): readonly Card[] {
  const out = [...deck];
  for (let i = out.length - 1; i > 0; i -= 1) {
    const j = random.nextInt(i + 1);
    if (!Number.isInteger(j) || j < 0 || j > i)
      throw new Error('RandomSource returned an invalid value');
...
export function deterministicDeck(ids: readonly string[]): readonly Card[] {
  ...
  if (new Set(ids).size !== ids.length)
    throw new Error('Deterministic deck contains duplicate cards');
  const chosen = ids.map(id => all.get(id));
  if (chosen.some(c => c === undefined))
    throw new Error('Deterministic deck contains an invalid card');
...
export function take(
  ...
  return [
    Object.freeze(deck.slice(count)),
    Object.freeze(deck.slice(0, count)),
  ];
}
