...
export type Command = Readonly<{ protocolVersion: number; tableId: string; actionId: string; expectedVersion: number; handId?: number; turnId?: string; type: 'action' | 'draw' | 'sit' | 'bot' | 'leave' | 'sitOut' | 'queueVariant' | 'sync'; action?: Action; indexes?: readonly number[]; seat?: number; variant?: Variant; botStyle?: BotStyle }>;
